 ...
 * @LastEditTime: 2024-04-28 13:22:09
...
static void vaddr_remove(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt){
    ...
    if(pf == PF_KERNEL) //内核虚拟内存池
    {
        ...
        struct task_struct* cur_thread = running_thread();
        bit_idx_start = (vaddr - cur_thread->userprog_vaddr.vaddr_startvaddr_start) / PG_SIZE;
        while (cnt<pg_cnt)
        {
            bitmap_set(&cur_thread->userprog_vaddr.vaddr_start.vaddr_bitmap,bit_idx_start++,cnt++,0);
        }
