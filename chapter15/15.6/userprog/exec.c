 ...
 * @Date: 2024-05-22 10:33:03
 * @LastEditors: Adward-DYX 1654783946@qq.com
 * @LastEditTime: 2024-05-22 13:15:09
 * @FilePath: /OS/chapter15/15.5/userprog/exec.c
...
/*将文件描述符fd指向的文件中，偏移为offset，大小为filesz的段加载到虚拟地址为vaddr的内存中*/
static bool segment_load(int32_t fd, uint32_t offset, uint32_t filesz, uint32_t vaddr){
    ...
    while(page_idx < occupy_pages){
        ...
        /*如果pde不存在，或者pte不存在就分配内存、pde的判断要在pte之前，否则pde若不存在会导致，判断pte时缺页异常*/
...
/*从文件系统上加载用户程序pathname，成功则返回层序的起始地址，否则返回-1*/
...
int32_t sys_execv(const char* path, const char* argv[]){
    ...
    /*exec不同于fork，为了使新进场更快被执行，直接从中断返回*/
