 ...
 * @Date: 2024-05-21 09:49:04
 * @LastEditors: Adward-DYX 1654783946@qq.com
 * @LastEditTime: 2024-05-24 13:59:32
 * @FilePath: /OS/chapter15/15.4/shell/shell.c
...
static void readline(char* buf, int32_t count){
    ...
    while(read(stdin_no,pos,1)!=-1 && (pos - buf) < count){ //在不出错的情况下，直到找到回车符才返回
        switch(*pos){
            /*找到回车或者换行符后认为键入的命令结束，直到返回*/
            case '\n':
            case '\r':
                *pos = 0;   //添加cmd_lin的终止字符0
                putchar('\n');
                return ;
...
static void cmd_execute(uint32_t argc, char** argv){
    if(!strcmp("ls", argv[0])){
        ...
        if(pid){    //父进程
            int32_t status;
            int32_t child_pid = wait(&status);//此时子进程若没有执行exit，my_shell会被阻塞，不在响应键入命令
            if(child_pid==-1){  //按理说程序正确的话不会执行到这句， fork 出的进程便是 shell 子进程
...
void my_shell(void){
    ...
    while(1){
        ...
        if(pipe_symbol){
            ...
            pipe(fd);
            /*将标准输出重定向到fd[1],是的后面的输出信息重定向到内核环形缓冲区*/
            fd_redirect(1,fd[1]);
