 ...
 * @Date: 2024-05-23 15:11:15
 * @LastEditors: Adward-DYX 1654783946@qq.com
 * @LastEditTime: 2024-05-24 11:19:18
 * @FilePath: /OS/chapter15/15.6/userprog/wait_exit.c
...
static void release_prog_resource(struct task_struct* release_thread){
    ...
    uint32_t* first_pte_vaddr_in_pde = NULL;    //用来记录pde中第一个pte指向的物理页起始地址
    ...
    while(pde_idx<user_pde_nr){
        ...
        pde = *v_pde_ptr;
        if(pde&0x00000001){ //如果页目录项p位为1，表示该页目录项可能有页表项
...
/*list_traversal的回调函数，查找pelem的parent_pid是否是ppid，成功则返回true,失败则返回false*/
static bool find_child(struct list_elem* pelem, int32_t ppid){
    ...
    if(pthread->parent_pid == ppid){    //若该任务的parent_pid为ppid，返回
        return true;    //list_traversal只有在回调函数返回ture时才会停止，继续遍历，所以在此返回true
    }
    ...
    while(1){
        ...
        if(child_elem!=NULL){
            ...
            *status = child_thread->exit_status;
            ...
            /* 从就绪队列和全部队列中删除进程表项 */
            thread_exit(child_thread,false);//传入false，是的thread_exit调用后回到此处
        ...
        if(child_elem == NULL){ //若没有子进程，则出错返回
            return -1;
        }else{
