 ...
 * @LastEditTime: 2024-05-14 15:35:38
 ...
 * 寄存器， intr_exit 中的出梭操作是此结构的逆操作
 * 此枝在线程自己的内核校中位置固定，所在页的最顶端
*/
struct intr_stack{
    uint32_t vec_no;    //kernel. S 宏 VECTOR 中 push %1压入的中断号
    uint32_t edi;
    uint32_t esi;
    uint32_t ebp;
    uint32_t esp_dummy;
    //虽然pushad把esp也压入，但是esp是不断变化的，所以会被popad忽略
    uint32_t ebx;
    uint32_t edx;
    uint32_t ecx;
    uint32_t eax;
    ...
    uint32_t err_code;  //err_code会陪压入在eip之后
    void (*eip)(void);
    uint32_t cs;
    uint32_t eflags;
    void* esp;
...
struct task_struct{
    ...
    int16_t parent_pid; //表示父进程的pid，也就是自己的父进程是谁
