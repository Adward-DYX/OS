 ...
 * @LastEditTime: 2024-05-23 10:46:59
...
static void* vaddr_get(enum pool_flags pf, uint32_t pg_cnt){
    ...
    uint32_t cnt = 0;
    if(pf==PF_KERNEL){
        bit_idx_start = bitmap_scan(&kernel_vaddr.vaddr_bitmap,pg_cnt); //  返回的是成功位置的下标需要页大小 12
        if(bit_idx_start == -1) return NULL;
        while(cnt < pg_cnt){
            bitmap_set(&kernel_vaddr.vaddr_bitmap,bit_idx_start + cnt++,1);
        }
        vaddr_start = kernel_vaddr.vaddr_start + bit_idx_start * PG_SIZE;   //kernel_vaddr.vaddr_start = 3222274048
    }else{
        //用户内存池
...
/*此函数的功能同 get_a_page 类似，只是少了虚拟地址池位图的操作*/
