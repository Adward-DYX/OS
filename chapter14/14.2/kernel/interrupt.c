...
static void idt_desc_init(void){
    ...
    make_idt_desc(&idtr[lastindex],IDT_DESC_ATTR_DPL3,syscall_handler);
...
static void pic_init(void){
    ...
    /*IRQ2用于级联从片，必须打开，否则无法响应从片上的中断。主片上打开的中断有 IRQO 的时钟，IRQl 的键盘和级联从片的IRQ2,其他全部关闭*/
...
static void general_intr_handler(uint8_t vec_nr){
    ...
    if(vec_nr==14){     
        ...
        asm ("movl %%cr2, %0" : "=r"(page_fault_vaddr));//cr2存放造成page_fault的地址
        put_str("\npage fault addr is ");put_int(page_fault_vaddr);
    }

    put_str("\n!!!!!!!!!!!!!!!!!!!!!! excetion message end!!!!!!!!!!!!!!!!!!!!!\n");
    //能进入中断处理程序就表示已经在关闭中断情况下
...
/*中断处理程序数组第 vector_no 个元素中注册安装中断处理程序 function*/
void register_handler(uint8_t vector_no, intr_handler function){
    //idt_table 数组中的函数是在进入中断后根据中断向量号调用的见 kernel/kernel.s 的 call [idt_table ＋ %1*4]
